:- object(rtec).
	...
	:- include('src/compiler.prolog').
	:- include('src/inputModule.prolog').
	:- include('src/processSimpleFluents.prolog').
	:- include('src/processSDFluents.prolog').
	:- include('src/processEvents.prolog').
	:- include('src/utilities/interval-manipulation.prolog').
	:- include('src/utilities/amalgamate-periods.prolog').
	% Load the dynamic grounding module
	:- include('src/dynamic grounding/dynamicGrounding.prolog').

	:- info([
		version is 0:1:0,
		author is 'Alexander Artikis et al, Paulo Moura et al',
		date is 2021-03-24,
		comment is 'Run-Time Event Calculus'
	]).
